...
const MonthlyExpenseChart = ({ monthlyChartData = [], range, setRange }) => {
  const topMonth = [...monthlyChartData].sort((a, b) => b.total - a.total)[0];
  ...
  return (
    <div className="relative overflow-hidden rounded-[32px] border border-white/10 bg-white/5 backdrop-blur-2xl p-4 shadow-2xl">
      ...
      <div className="flex items-center gap-4 mb-6 relative z-10">
        ...
        <div>
          <h3 className="text-3xl font-black text-white">ค่าใช้จ่ายรายเดือน</h3>

          <div className="text-sm text-gray-300">ภาพรวมค่าใช้จ่ายรายเดือน</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="rounded-3xl bg-white/5 border border-white/10 p-5 backdrop-blur-xl">
          <div className="text-sm text-gray-300">เดือนใช้สูงสุด</div>
          ...
          <div className="text-sm text-white/70 mt-1">
            ฿{Number(topMonth?.total || 0).toLocaleString()}
          </div>
        </div>

        <div className="rounded-3xl bg-white/5 border border-white/10 p-5 backdrop-blur-xl">
          <div className="text-sm text-gray-300">เฉลี่ยต่อเดือน</div>

          <div className="text-2xl font-black text-cyan-300 mt-2">
            ฿{Math.round(averageMonthly).toLocaleString()}
          </div>

          <div className="text-sm text-white/70 mt-1">Monthly Average</div>
        </div>
      </div>
